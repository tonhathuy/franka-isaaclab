...
def two_cubes_stacked(
    ...
    """Check if 2 cubes are stacked successfully."""
    robot: Articulation = env.scene[robot_cfg.name]
    cube_1: RigidObject = env.scene[cube_1_cfg.name]
    cube_2: RigidObject = env.scene[cube_2_cfg.name]
    ...
    # Check cube positions
    stacked = torch.logical_and(xy_dist < xy_threshold, h_dist - height_diff < height_threshold)
    stacked = torch.logical_and(pos_diff[:, 2] < 0.0, stacked)
